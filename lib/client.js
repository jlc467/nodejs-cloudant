...
const debug = _debug('cloudant:client');
const debugAtms = _debug('atms');
...
class CloudantClient {
  ...
  request(options, callback) {
    ...
    async.forever(function(done) {
      ...
      var optionsString = JSON.stringify(request.options)
      
      var optionHash = require("crypto")
        .createHash("md5")
      ...
      var toLog = {
        ...
        ...request.options,
        ...request.state,
        isGlobalQuery: !optionsString.includes('/_partition/')
       }
    
      if (request.state.attempt > 1 || toLog.isGlobalQuery) {
          debugAtms(`Request attempt: ${request.state.attempt} %j`, toLog);
